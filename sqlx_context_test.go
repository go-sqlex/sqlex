...
func RunWithSchemaContext(ctx context.Context, schema Schema, t *testing.T, test func(ctx context.Context, db *DB, t *testing.T)) {
	runner := func(ctx context.Context, db *DB, t *testing.T, create, drop, now string) {
		...
		// Clean up leftover tables first (previous test run may have been interrupted), then create, ensuring idempotency
		// MultiExecContext(ctx, db, drop)
...
func loadDefaultFixtureContext(ctx context.Context, db *DB, t *testing.T) {
	...
	tx.MustExecContext(ctx, "INSERT INTO place (country, city, telcode) VALUES (?, ?, ?)", "United States", "New York", "1")
	tx.MustExecContext(ctx, "INSERT INTO place (country, telcode) VALUES (?, ?)", "Hong Kong", "852")
	tx.MustExecContext(ctx, "INSERT INTO place (country, telcode) VALUES (?, ?)", "Singapore", "65")
	if db.DriverName() == "mysql" {
		tx.MustExecContext(ctx, "INSERT INTO capplace (`COUNTRY`, `TELCODE`) VALUES (?, ?)", "Sarf Efrica", "27")
	} else {
		tx.MustExecContext(ctx, "INSERT INTO capplace (\"COUNTRY\", \"TELCODE\") VALUES (?, ?)", "Sarf Efrica", "27")
	}
	tx.MustExecContext(ctx, "INSERT INTO employees (name, id) VALUES (?, ?)", "Peter", "4444")
	tx.MustExecContext(ctx, "INSERT INTO employees (name, id, boss_id) VALUES (?, ?, ?)", "Joe", "1", "4444")
	tx.MustExecContext(ctx, "INSERT INTO employees (name, id, boss_id) VALUES (?, ?, ?)", "Martin", "2", "4444")
	tx.Commit()
}
...
func TestMissingNamesContext(t *testing.T) {
	RunWithSchemaContext(context.Background(), defaultSchema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		// test Select first
		// Default behavior is unsafe mode: silently ignore missing destination fields without error
		pps := []PersonPlus{}
		// pps lacks added_at destination, but should succeed (default unsafe)
		err := db.SelectContext(ctx, &pps, "SELECT * FROM person")
		if err != nil {
			t.Error("Select with missing dest field should succeed in default unsafe mode:", err)
		}
		...
		if err != nil {
			t.Error("Get with missing dest field should succeed in default unsafe mode:", err)
		}
		...
		if err != nil {
			t.Error("StructScan with missing dest field should succeed in default unsafe mode:", err)
		}
		rowsx.Close()
...
func TestEmbeddedStructsContext(t *testing.T) {
...
func TestNilInsertsContext(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), schema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		db.MustExecContext(ctx, `INSERT INTO tt (id) VALUES (1)`)
		db.GetContext(ctx, &v, `SELECT * FROM tt`)
		if v.ID != 1 {
		...
		db.GetContext(ctx, &v2, `SELECT * FROM tt WHERE id=2`)
		if v.ID != v2.ID {
...
func TestScanErrorContext(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), schema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		_, err := db.Exec("INSERT INTO kv (k, v) VALUES (?, ?)", "hi", 1)
		if err != nil {
			t.Error(err)
		}
...
func TestUsageContext(t *testing.T) {
	RunWithSchemaContext(context.Background(), defaultSchema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		err = db.GetContext(ctx, &jason, "SELECT * FROM person WHERE first_name=?", "Foobar")
		if err == nil {
		...
		stmt1, err := db.PreparexContext(ctx, "SELECT * FROM person WHERE first_name=?")
		if err != nil {
			t.Fatal(err)
		}
		jason = Person{}
		...
		stmt2, err := db.PreparexContext(ctx, "SELECT * FROM person WHERE first_name=?")
		if err != nil {
			t.Fatal(err)
		}
		jason = Person{}
		...
		stmt, err := db.PreparexContext(ctx, "SELECT country, telcode FROM place WHERE telcode > ? ORDER BY telcode ASC")
		if err != nil {
			t.Error(err)
		}
		...
		stmt, err = db.PreparexContext(ctx, "SELECT * FROM person WHERE first_name=?")
		if err != nil {
			t.Fatal(err)
		}
...
// tests that sqlex will not panic when the wrong driver is passed because
// of an automatic nil dereference in sqlex.Open(), which was fixed.
...
func TestInContext(t *testing.T) {
	...
	tests = []tr{
		...
		{"SELECT * FROM foo WHERE x = ? and y = ?",
			...
			0},
		// Note: Since sqlex Phase 1.9, "WHERE x = ? + empty slice" no longer errors —
		// non-(?) form + empty slice will be sent to the driver as the entire slice (see README slice handling section).
		// The original sqlx test case "empty slice in non-IN position should error" has been removed.
...
func TestEmbeddedLiteralsContext(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), schema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		db.MustExecContext(ctx, "INSERT INTO x (k) VALUES (?), (?), (?);", "one", "two", "three")

		target := t1{}
		err := db.GetContext(ctx, &target, "SELECT * FROM x WHERE k=?", "one")
		if err != nil {
			t.Error(err)
		}
		if *target.K != "one" {
		...
		target2 := t2{}
		err = db.GetContext(ctx, &target2, "SELECT * FROM x WHERE k=?", "one")
		if err != nil {
			t.Error(err)
		}
		if *target2.K != "one" {
...
func TestConn(t *testing.T) {
	...
	RunWithSchemaContext(context.Background(), schema, t, func(ctx context.Context, db *DB, t *testing.T) {
		...
		_, err = conn.ExecContext(ctx, `INSERT INTO tt_conn (id, value) VALUES (?, ?), (?, ?)`, 1, "a", 2, "b")
		if err != nil {
			t.Fatal(err)
		}
		...
		v1 := s{}
		err = conn.GetContext(ctx, &v1, "SELECT * FROM tt_conn WHERE id=?", 1)
		...
		stmt, err := conn.PreparexContext(ctx, "SELECT * FROM tt_conn WHERE id=?")
		if err != nil {
			t.Fatal(err)
		}
		v1 = s{}
