...
/* compile time checks that Db, Tx implement expected interfaces */
var _, _ Ext = &DB{}, &Tx{}
...
func connectAll() {
	...
	// Empty string is treated as unconfigured; skip the corresponding database; explicitly set to "skip" also skips
	TestPostgres = pgdsn != "" && pgdsn != "skip"
	TestMysql = mydsn != "" && mydsn != "skip"
	TestSqlite = sqdsn != "skip" // SQLite allows empty DSN; defaults to :memory: below

	// Default to in-memory database when SQLite is not configured
	if sqdsn == "" {
		sqdsn = ":memory:"
	}

	// Only append if DSN is non-empty and does not already contain parseTime (avoid duplicate appending)
...
// if we've used Place already somewhere in sqlex
type CPlace Place
...
func loadDefaultFixture(db *DB, t *testing.T) {
	...
	if db.DriverName() == "mysql" {
		tx.MustExec("INSERT INTO capplace (`COUNTRY`, `TELCODE`) VALUES (?, ?)", "Sarf Efrica", "27")
	} else {
		tx.MustExec("INSERT INTO capplace (\"COUNTRY\", \"TELCODE\") VALUES (?, ?)", "Sarf Efrica", "27")
	}
	tx.MustExec("INSERT INTO employees (name, id) VALUES (?, ?)", "Peter", "4444")
	tx.MustExec("INSERT INTO employees (name, id, boss_id) VALUES (?, ?, ?)", "Joe", "1", "4444")
	tx.MustExec("INSERT INTO employees (name, id, boss_id) VALUES (?, ?, ?)", "Martin", "2", "4444")
	tx.Commit()
}
...
func TestNilInserts(t *testing.T) {
	...
	RunWithSchema(schema, t, func(db *DB, t *testing.T, now string) {
		...
		db.MustExec(`INSERT INTO tt (id) VALUES (1)`)
		db.Get(&v, `SELECT * FROM tt`)
		if v.ID != 1 {
		...
		db.Get(&v2, `SELECT * FROM tt WHERE id=2`)
		if v.ID != v2.ID {
...
func TestMultiInsert(t *testing.T) {
	RunWithSchema(defaultSchema, t, func(db *DB, t *testing.T, now string) {
		loadDefaultFixture(db, t)
		q := `INSERT INTO employees (name, id) VALUES (?, ?), (?, ?);`
		db.MustExec(q,
			"Name1", 400,
			"name2", 500,
		)
...
func TestUsage(t *testing.T) {
	RunWithSchema(defaultSchema, t, func(db *DB, t *testing.T, now string) {
		...
		err = db.Get(&jason, "SELECT * FROM person WHERE first_name=?", "Foobar")
		if err == nil {
		...
		stmt1, err := db.Preparex("SELECT * FROM person WHERE first_name=?")
		if err != nil {
			t.Fatal(err)
		}
		jason = Person{}
		...
		stmt2, err := db.Preparex("SELECT * FROM person WHERE first_name=?")
		if err != nil {
			t.Fatal(err)
		}
		jason = Person{}
		...
		stmt, err := db.Preparex("SELECT country, telcode FROM place WHERE telcode > ? ORDER BY telcode ASC")
		if err != nil {
			t.Error(err)
		}
		...
		stmt, err = db.Preparex("SELECT * FROM person WHERE first_name=?")
		if err != nil {
			t.Fatal(err)
		}
...
func TestEmbeddedLiterals(t *testing.T) {
	...
	RunWithSchema(schema, t, func(db *DB, t *testing.T, now string) {
		...
		db.MustExec("INSERT INTO x (k) VALUES (?), (?), (?);", "one", "two", "three")

		target := t1{}
		err := db.Get(&target, "SELECT * FROM x WHERE k=?", "one")
		if err != nil {
			t.Error(err)
		}
		if *target.K != "one" {
		...
		target2 := t2{}
		err = db.Get(&target2, "SELECT * FROM x WHERE k=?", "one")
		if err != nil {
			t.Error(err)
		}
		if *target2.K != "one" {
